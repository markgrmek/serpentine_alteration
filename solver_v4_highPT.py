...
single_lookup = read_csv(f'single_output_highPT.csv')
...
T: float =  800 + 273.15
WCO2_B: float = 0.0022 #0.004   #CO2_s in serpentine
WCO2_SOAP: float = 0.022 #0.04715  #CO2_s in soapstone
...
L = 1 #domain length (m)
N_cells: int = 100 #number of cells
dx = L/N_cells  #step in x (m)
x: ndarray[float] = linspace(0,L,N_cells+1) #discrete domain

time_tot: float = 10*L**2/DCO2_B #total integration time (s)
...
for time_step in range(N_steps):
    ...
    #PRINT RESULTS-----------------------------------
    if time_step % 300 == 0: #print out result of each time step
        print(f"""
    ...
    if time == time_tot or time_step==(N_steps-1):
        ...
        savetxt(fname=f'grid_highPT.csv', X=x, delimiter=',', fmt='%f')
        savetxt(fname=f'time_highPT.csv', X=time_output, delimiter=',', fmt='%.18e')
        savetxt(fname=f'wCO2_s_highPT.csv',X=wCO2_s_output,delimiter=',',fmt='%.18e')
        savetxt(fname=f'wCO2_fl_highPT.csv',X=wCO2_fl_output,delimiter=',',fmt='%.18e')
        savetxt(fname=f'por_highPT.csv',X=por_output,delimiter=',',fmt='%.18e')
        savetxt(fname=f'density_highPT.csv',X=density_output,delimiter=',',fmt='%.18e')
        savetxt(fname=f'wSiO2_s_highPT.csv',X=wSiO2_s_output,delimiter=',',fmt='%.18e')
        savetxt(fname=f'wH2O_s_highPT.csv',X=wH2O_s_output,delimiter=',',fmt='%.18e')
        savetxt(fname=f'dolomite_highPT.csv',X=dolomite_output,delimiter=',',fmt='%.18e')
        savetxt(fname=f'talc_highPT.csv',X=talc_output,delimiter=',',fmt='%.18e')
        savetxt(fname=f'magnesite_highPT.csv',X=magnesite_output,delimiter=',',fmt='%.18e')
        savetxt(fname=f'chlorite_highPT.csv',X=chlorite_output,delimiter=',',fmt='%.18e')
        savetxt(fname=f'orthopyroxene_highPT.csv',X=orthopyroxene_output,delimiter=',',fmt='%.18e')
        savetxt(fname=f'olivine_highPT.csv',X=olivine_output,delimiter=',',fmt='%.18e')
        savetxt(fname=f'quartz_highPT.csv',X=quartz_output,delimiter=',',fmt='%.18e')
        savetxt(fname=f'corundum_highPT.csv',X=corundum_output,delimiter=',',fmt='%.18e')
        savetxt(fname=f'hematite_highPT.csv',X=hematite_output,delimiter=',',fmt='%.18e')
        savetxt(fname=f'lime_highPT.csv',X=lime_output,delimiter=',',fmt='%.18e')
        savetxt(fname=f'calcite_highPT.csv',X=calcite_output,delimiter=',',fmt='%.18e')
        break
